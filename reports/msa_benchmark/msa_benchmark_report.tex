...
\subsection{Temperature Continuation (Annealing) Methodology for Low Temperatures}

At strongly coupled thermodynamic states ($T^* \le 0.1$, corresponding to $\beta \mu^2 \ge 10.0$), the sharp spatial gradients at the hard-core boundary $r = \sigma$ and large amplitude of the long-range dipolar tail make direct fixed-point iterations with naive zero initial guesses ($\mathbf{c}^{(0)} = \mathbf{0}$) susceptible to numerical stalling or divergence. To robustly resolve low-temperature states and track the equilibrium solution manifold, we have implemented a geometric temperature continuation (annealing) framework.

\subsubsection{Mathematical Formulation and Continuation Schedule}

Given a high initial temperature $T_{\text{start}}^*$ (where fixed-point convergence is guaranteed and rapid) and a target low temperature $T_{\text{target}}^*$, the parameter space is discretized into $N$ logarithmic continuation stages $s \in \{0, 1, \dots, N-1\}$:
\begin{equation}
    T_s^* = T_{\text{start}}^* \left( \frac{T_{\text{target}}^*}{T_{\text{start}}^*} \right)^{\frac{s}{N-1}}, \quad \beta_s \mu^2 = \frac{1}{T_s^*}.
\end{equation}
For each stage $s$:
\begin{enumerate}
    \item \textbf{Warm Starting}: The converged direct correlation matrix $\mathbf{c}_s(r)$ and screening potential $\bm{\eta}_s(r)$ from stage $s$ serve as the initial state for stage $s+1$, preserving the smooth non-linear core polarization profile.
    \item \textbf{Regularized Difference Anderson Acceleration}: Within each stage, history residuals $\mathbf{d}_j = \mathbf{c}_{j}^{\text{new}} - \mathbf{c}_j$ are accumulated. A scale-invariant Tikhonov-regularized least-squares problem is solved for the difference vectors $\Delta \mathbf{d}_j = \mathbf{d}_j - \mathbf{d}_{K}$:
    \begin{equation}
        \sum_{k=0}^{K-1} \left( \langle \Delta \mathbf{d}_j, \Delta \mathbf{d}_k \rangle + \lambda \delta_{jk} \right) \gamma_k = -\langle \Delta \mathbf{d}_j, \mathbf{d}_K \rangle, \quad \lambda = 10^{-4} \max_{j} \langle \Delta \mathbf{d}_j, \Delta \mathbf{d}_j \rangle.
    \end{equation}
    The optimal extrapolation $\mathbf{c}_{s}^{(k+1)} = \sum_{j=0}^{K} \gamma_j (\mathbf{c}_j + \alpha \mathbf{d}_j)$ is bounded to ensure numerical contractivity.
    \item \textbf{Adaptive Multi-Stage Tolerances}: Intermediate stages use a relaxed tolerance ($\varepsilon_{\text{stage}} = 10^{-4}$) to rapidly advance along the continuation path, while the final stage enforces high precision ($\varepsilon_{\text{target}} = 10^{-6}$).
\end{enumerate}

\begin{table}[htbp]
    \centering
    \caption{Performance and convergence characteristics of the Temperature Continuation solver for dipolar hard spheres at $\phi = 0.10$, transitioning from $T_{\text{start}}^* = 10.0$ down to $T_{\text{target}}^* = 0.10$ ($N=10$ stages).}
...
As shown in Table~\ref{tab:continuation}, the continuation solver traverses two orders of magnitude in dipolar coupling strength in under $3$ seconds total runtime. Each intermediate stage resolves in only 8--14 iterations, and the final state reaches micro-precision ($\mathcal{L}_2 = 7.69 \times 10^{-7}$). 
...
\subsection{Parameter Space Heatmaps across \texorpdfstring{$(\phi, T^*)$}{(phi, T*)}}

To provide a comprehensive assessment across the full two-dimensional thermodynamic phase space $(\phi, T^*)$, Figure~\ref{fig:heatmap} presents logarithmic color maps of the Root Mean Square Error (RMSE) for all structure factor components.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{plots/fig6_rmse_heatmap.pdf}
    \caption{Two-dimensional RMSE heatmaps across the $(\phi, T^*)$ thermodynamic parameter space for: (a) dipolar projection $S^{110}(k)$, (b) anisotropic projection $S^{112}(k)$, (c) decoupled mode $S^1(k)$, and (d) base hard-core projection $S^{000}(k)$. Exact numerical values are annotated in each grid cell.}
    \label{fig:heatmap}
\end{figure}

The heatmap visualization clearly highlights the three characteristic operational regimes:
\begin{enumerate}
    \item \textbf{Micro-Accuracy High-Temperature Regime ($T^* = 10.0$)}: Across all packing fractions $\phi \in [0.1, 0.5]$, the solver achieves errors on the order of $\text{RMSE} \sim 10^{-6}$--$10^{-5}$ (bright yellow cells), proving exact analytical agreement.
    \item \textbf{Moderate Coupling Regime ($T^* = 1.0$)}: Errors remain low ($\text{RMSE} \sim 10^{-4}$--$10^{-3}$, teal green cells), with a slight monotonic increase at higher density reflecting hard-core boundary steepening.
    \item \textbf{Ultra-Strong Coupling Regime ($T^* \le 0.01$)}: The Wertheim pole singularity ($1 - \frac{\rho}{3}C^1 \to 0$) renders fixed-point Picard iterations linearly unstable, illustrating the necessity of Newton-Krylov methods for sub-critical dielectric transitions.
    \item \textbf{Decoupled Reference Base ($S^{000}$)}: Panel (d) demonstrates strict temperature independence of $S^{000}(k)$ errors, validating the exact separation of isotropic excluded-volume and anisotropic electrostatic channels in MSA theory.
\end{enumerate}
